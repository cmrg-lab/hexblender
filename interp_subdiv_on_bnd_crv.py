...
1.4 
    Return boolean array nOnBndCrv if flag returnnOnBndCrv is set.            
...
def interp_subdiv(e,n,returnnOnBndCrv=False,linearNearBndCrv=False):

    print("!! Using interp_subdiv_on_bnd_crv")
    if e.min()==1:
        e = e-1
    ...
    edgesUsedOnce = edges[numpy.nonzero(usedOnce)[0],:]
    
    
    nOnBndCrv = numpy.zeros((num_n,1),int)
    nOnBndCrv[edgesUsedOnce[:,0]] = 1
    nOnBndCrv[edgesUsedOnce[:,1]] = 1    
    ...
    nOnBndCrv = numpy.logical_and(nOnBndCrv,~isCorner)
    nOnBndCrv = numpy.array(nOnBndCrv,int)
    
    if linearNearBndCrv:
        # Find edges connected to boundary edges that are not boundary edges and adjust the points on those edges
        
        ePerpM = {} 
        
        edgePerpToBndEdge = numpy.zeros(edges.shape[0],bool)
        for k in range(edges.shape[0]):
            if (nOnBndCrv[edges[k,0]] + nOnBndCrv[edges[k,1]])==1:
                edgePerpToBndEdge[k] = True
                eInd = (min(edges[k,0],edges[k,1]),max(edges[k,0],edges[k,1]))
                ePerpM[eInd] = 1
                ePn[eInd] = .5 * (n[edges[k,0],:]+n[edges[k,1],:])
        
        # Find the face points neighboring a boundary edge and adjust the point to be linearly interpolated in a
        # direction perpendicular to the curve.
        for k in range(fExt.shape[0]):
            if numpy.logical_and((min(fExt[k,0],fExt[k,1]),max(fExt[k,0],fExt[k,1])) in ePerpM,
                                 (min(fExt[k,2],fExt[k,3]),max(fExt[k,2],fExt[k,3])) in ePerpM ):
                face = fExt[k,:].copy()
                face.sort()
                fInd = tuple(face)
                import pdb; pdb.set_trace()
                fPn[fInd] =  .5 * (ePn[(min(fExt[k,0],fExt[k,3]),max(fExt[k,0],fExt[k,3]))] + ePn[(min(fExt[k,1],fExt[k,2]),max(fExt[k,1],fExt[k,2]))])
            
            if numpy.logical_and((min(fExt[k,0],fExt[k,3]),max(fExt[k,0],fExt[k,3])) in ePerpM,
                                (min(fExt[k,1],fExt[k,2]),max(fExt[k,1],fExt[k,2])) in ePerpM):
                face = fExt[k,:].copy()
                face.sort()
                fInd = tuple(face)
                fPn[fInd] =  .5 * (ePn[(min(fExt[k,0],fExt[k,1]),max(fExt[k,0],fExt[k,1]))] + ePn[(min(fExt[k,2],fExt[k,3]),max(fExt[k,2],fExt[k,3]))])

    if returnnOnBndCrv:
        return [fPn,ePn,numpy.array(nOnBndCrv,'bool')]
    else:
        return [fPn,ePn]
